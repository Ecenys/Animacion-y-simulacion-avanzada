...
public class Tetrahedron : MonoBehaviour
{
    ...
    {
        double m = this.ComputeLameParameterFirst();
        double l = this.ComputeLameParameterSecond();

        MatrixXD e = this.ComputeCauchyStrain();

        return Volume0 * (m * (e.Transpose() * e).Trace() + (l / 2) * e.Trace() * e.Trace());
    }
    ...
    public VectorXD ComputeForce()
    {
        ...
        MatrixXD P = 2 * m * e + l * e.Trace() * DenseMatrixXD.CreateIdentity(3);
    ...
    public MatrixXD ComputeDfDx()
    {
        ...
        dPdF[0, 0] = dPdF[4, 4] = dPdF[8, 8] = 2 * m + l;
        dPdF[4, 0] = dPdF[8, 0] = l;
        dPdF[0, 4] = dPdF[8, 4] = l;
        dPdF[0, 8] = dPdF[4, 8] = l;
    ...
    public void UpdateEmbedded()
    {
        ...
        {
            embeddedPoints[i].position = ComputeShapeFunctionValue(
                embeddedPoints[i].isoParam)[0] * Utils.ToVectorXD(this.Nodes[0].Pos) + 
                ComputeShapeFunctionValue(embeddedPoints[i].isoParam)[1] * Utils.ToVectorXD(this.Nodes[1].Pos) + 
                ComputeShapeFunctionValue(embeddedPoints[i].isoParam)[2] * Utils.ToVectorXD(this.Nodes[2].Pos) + 
                ComputeShapeFunctionValue(embeddedPoints[i].isoParam)[3] * Utils.ToVectorXD(this.Nodes[3].Pos
                );
        }
    }
    ...
    public bool TryAddEmbedded(FEMSystem.EmbeddedPoint emb)
    {
        ...
        emb.isoParam = TransformRest2Isoparametric(position0);
        if (IsWithinElement(emb.isoParam))
        {
            embeddedPoints.Add(emb);
            emb.position = emb.isoParam;
    ...
    public void AddCollisionForceEmbedded(VectorXD force, double k, VectorXD p, VectorXD n)
    {
        ...
        {
            // COMPLETAR Parte 3
            float projDist = Vector3.Dot(Utils.ToVector3(embeddedPoints[i].position - p), Utils.ToVector3(n));
            if(projDist <= 0)
                force.SetSubVector(Nodes[i].index, 3, force.SubVector(Nodes[i].index, 3) + -k * projDist * n);
            else
                force.SetSubVector(Nodes[i].index, 3, Utils.ToVectorXD(Vector3.zero));
        }
    }
