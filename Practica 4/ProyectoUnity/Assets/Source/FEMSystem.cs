...
public class FEMSystem : Simulable
{
    ...
    public override void SetPosition(VectorXD position)
    {
        ...
        if (this.embededMesh != null && this.embeddedPoints.Count == this.embededMesh.vertexCount)
        {
            foreach (Tetrahedron tetra in Tetras)
            {
    ...
    bool EmbedMesh(Mesh mesh, List<EmbeddedPoint> embeddedPoints)
    {
        for (int i = 0; i < mesh.vertexCount; ++i)
        {
            ...
            EmbeddedPoint eP = new EmbeddedPoint();
            eP.position = Utils.ToVectorXD(mesh.vertices[i]);


            for (int j = 0; j < Tetras.Count; j++)
            {
                if (Tetras[j].TryAddEmbedded(eP))
                {
                    found = true;
                    break;
                }
    ...
    void AddCollisionForceEmbedded(VectorXD force)
    {
        ...
        { 
            ...
            for (int i = 0; i < Nodes.Count; ++i)
            {
                ...
                if (projDist < 0.01)
                {
                     Tetras[i].AddCollisionForceEmbedded(
                         force,
                         k,
                         Utils.ToVectorXD(p),
                         Utils.ToVectorXD(n)
    ...
    void AddCollisionJacobianEmbedded(MatrixXD dfdx, MatrixXD dfdv)
    {
        ...
        for (int o = 0; o < this.CollObstacles.Count; ++o)
        {
            ...
            for (int i = 0; i < Nodes.Count; ++i)
            {
                ...
                if (projDist < 0.01)
                {
                    Tetras[i].AddCollisionJacobianEmbedded(
                        dfdx,
                        dfdv,
                        k,
                        Utils.ToVectorXD(p),
